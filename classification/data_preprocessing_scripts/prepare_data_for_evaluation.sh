...
do
    ...
    DATASET_NAME="${dataset_name}" \
    OUTPUT_FILE="${classification_folder}/${dataset_name}/${dataset_name}_keyphrase_expansion_retrieval_based_ukg_custom_trained_combined_references_nounphrase_v6-${i}_position_penalty+length_penalty.json" \
    python prepare_data_for_evaluation.py
    ...
    DATASET_NAME="${dataset_name}" \
    OUTPUT_FILE="${classification_folder}/${dataset_name}/${dataset_name}_keyphrase_expansion_autokeygen-${i}.json" \
    python prepare_data_for_evaluation.py

    KEYPHRASE_EXPANSION_PATH="/scratch/lamdo/precompute_keyphrase_extraction/${dataset_name}--copyrnn-${i}.json" \
    DATASET_NAME="${dataset_name}" \
    OUTPUT_FILE="${classification_folder}/${dataset_name}/${dataset_name}_keyphrase_expansion_copyrnn-${i}.json" \
    python prepare_data_for_evaluation.py

    KEYPHRASE_EXPANSION_PATH="/scratch/lamdo/precompute_keyphrase_extraction/${dataset_name}--uokg-${i}.json" \
    DATASET_NAME="${dataset_name}" \
    OUTPUT_FILE="${classification_folder}/${dataset_name}/${dataset_name}_keyphrase_expansion_uokg-${i}.json" \
    python prepare_data_for_evaluation.py
done