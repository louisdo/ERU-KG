...
class SimpleDataset:
    ...
    def __init__(self, data_path: Union[str, tuple], sep_token: str, batch_size=32,
                 fields: List = None, key: str = None, processing_fn=None, embed=False):
        ...
        if not processing_fn:
            if embed:
                ...
                if type(data_path[0]) == str and os.path.isdir(data_path[0]):
                    self.data = datasets.load_from_disk(data_path[0])
                else:
                    self.data = datasets.load_dataset(data_path[0], data_path[1], split="evaluation")   
        else:
