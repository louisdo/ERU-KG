...
class Evaluator:
    def __init__(self, name: str, meta_dataset: Union[str, tuple], dataset_class, model: Model, batch_size: int,
                 fields: list, key: str = None, process_fn=None, embed=False):
        if model:
            if type(model) != list:
                model = [model]
            for m in model:
            ...
            datasets = [dataset_class(meta_dataset, m.tokenizer.sep_token, batch_size, fields, key,
                                    process_fn, embed) for m in model]
...
class SupervisedEvaluator(Evaluator):
    ...
    def evaluate(self, embeddings, **kwargs):
        ...
        if type(self.test_dataset) == str and os.path.isdir(self.test_dataset):
            ...
            split_dataset = datasets.load_from_disk(self.test_dataset)
        else:
    ...
    def classify(self, x_train: np.ndarray, x_test: np.ndarray, y_train: np.ndarray, cv: int = 3,
        ...
        if self.task == SupervisedTask.MULTILABEL_CLASSIFICATION:
            estimator = LinearSVC(max_iter=10000, verbose=True)
            # svm = GridSearchCV(estimator=estimator, cv=cv, param_grid={'C': Cs}, n_jobs=8)
            # svm = OneVsRestClassifier(svm, n_jobs=8, verbose=1)
            svm = OneVsRestClassifier(estimator, n_jobs=8, verbose=1)
        else:
