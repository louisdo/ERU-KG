import json, random, pickle, os, torch, nltk
...
with open("/scratch/lamdo/unsupervised_keyphrase_prediction_2022/data/unsupervised/phrases_counter_kp20k.json") as f:
    phrase_counter = json.load(f)
    
    
word_counter = Counter()
for phrase in tqdm(phrase_counter):
    words = nltk.word_tokenize(phrase)
    for w in words:
        word_counter[w] += phrase_counter[phrase]
...
def select_keyphrases(keyphrases, check_if_in_corpus = False):
    flag = False
    res = []
    for keyphrase in keyphrases:
        if len(keyphrase.split(" ")) == 1 and flag == False:
            flag = True
            res.append(keyphrase)
        elif len(keyphrase.split(" ")) >= 2:
            res.append(keyphrase)
    if check_if_in_corpus == True:
        res = [item for item in res if keep_if_in_corpus(item)]
    return res




def keep_if_in_corpus(phrase):
    return phrase in PHRASE_COUNTER
...
def init_model(model_run_index):
    ...
    if MODELS.get("model_run_index") != model_run_index:
        CKPT_PATH = f"/scratch/lamdo/unsupervised_keyphrase_prediction_2022/previous_work/autokeygen/data/checkpoints/final/{model_run_index}/autokeygen.pth" # "./data/checkpoints/final/1/autokeygen.pth" "../../data/supervised_checkpoints/final/1/supervised.pth"
