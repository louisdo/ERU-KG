...
processed_data = []
for SPLIT_NAME in ["search", "high_influence_cite"]:
    ds = load_dataset("allenai/scirepeval", SPLIT_NAME)

    len_train_split = len(ds["train"])
    one_third_len_train_split = int(len_train_split / 3)
    print(SPLIT_NAME, f"Data length {one_third_len_train_split}")
    for i, line in tqdm(enumerate(ds["train"]), desc = "Get data from train split"):
        if i > one_third_len_train_split: break
...
create_tsv(data = processed_data[:], file_name = "/home/lamdo/keyphrase_informativeness_test/splade/data/keyphrase_informative_scirepeval/triplets/raw.tsv")