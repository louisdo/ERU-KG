...
# class SpladeSoftPlus(SiameseBase):
#     """SPLADE model
#     """
...
#         print("INFO: Initializing SpladeSoftPlus model!!!")
#         self.output_dim = self.transformer_rep.transformer.config.vocab_size  # output dim = vocab size = 30522 for BERT
#         assert agg in ("sum", "max")
#         self.agg = agg
...
#         out = self.encode_(tokens, is_q)["logits"]  # shape (bs, pad_len, voc_size)
#         if self.agg == "sum":
#             return torch.sum(torch.log(1 + torch.nn.functional.softplus(out)) * tokens["attention_mask"].unsqueeze(-1), dim=1)
#         else:
#             values, _ = torch.max(torch.log(1 + torch.nn.functional.softplus(out)) * tokens["attention_mask"].unsqueeze(-1), dim=1)
#             return values
#             # 0 masking also works with max because all activations are positive
