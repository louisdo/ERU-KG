...
with open(INPUT_FILE) as f:
    ...
    for line in tqdm(f):
        ...
        title = jline["title"].lower()
        abstract = jline["abstract"].lower()
        text = f"{title}\n{abstract}"
        corpus.append(text)
...
documents = convert_dataset_to_pyserini_format(dataset = corpus, ids = ids, extract_nounphrases=True, keyword_for_document_expansion_path=KEYWORD_FOR_DOCUMENT_EXPANSION)
